...
function readYamlThresholds(raw) {
  ...
  return { strict: get('strict', 0.9), review: get('review', 0.8), auto: get('auto', 0.0) };
}
...
async function loadDB() {
  try { return JSON.parse(await fsp.readFile(DB_PATH(), 'utf8')); }
  catch { return { participants: [], sessions: [] }; }
}
...
async function loadRawReviewData(participantId) {
  ...
  // Dynamically find the latest timestamped consensus TSV
...
ipcMain.handle('pipeline:detect-stages', async (_e, { sessionId }) => {
  ...
  for (let i = 0; i < PIPELINE_STAGES.length; i++) {
    ...
    const outputExists = await stageOutputExists(outputDir, s.outputCheck, session.participantId);
    const canRun = i === 0 ? true : await stageOutputExists(outputDir, PIPELINE_STAGES[i - 1].outputCheck, session.participantId);
    // For stage 3: also report whether reviewer TSVs are present
    let reviewedTsvPresent = false;
    if (s.id === 'stage3') {
      const reviewDir = path.join(outputDir, 'review_files');
      const reviewFiles = await listReviewerFiles(reviewDir, session.participantId);
      reviewedTsvPresent = reviewFiles.length > 0;
    }
    results.push({ id: s.id, label: s.label, outputExists, canRun, reviewedTsvPresent });
  }
  return results;
});
...
ipcMain.handle('pipeline:run', async (_e, { sessionId, stages, whisperModel, stage3Mode = 'all', stage3Reviewers = null }) => {
  if (runningProc) throw new Error('A pipeline run is already in progress.');

  const session = await resolveSession(sessionId);
  if (!session) throw new Error('Session not found.');

  const stagesToRun = stages && stages.length > 0
  ...
  const db = await loadDB();
  const s0 = db.sessions.find((s) => s.id === sessionId);
  if (s0) { s0.pipelineStatus = 'running'; s0.pipelineRunAt = new Date().toISOString(); await saveDB(db); }
  ...
  for (const stage of stagesToRun) {
    ...
    if (stage.id === 'stage3') {
      // ----------------------------------------------------------------
      // Reviewer file selection
      //   stage3Mode:      'none' | 'all' | 'select'
      //   stage3Reviewers: null (all) | [] (none) | string[] (selected)
      // ----------------------------------------------------------------
      ...
      if (stage3Mode === 'none' || (Array.isArray(stage3Reviewers) && stage3Reviewers.length === 0)) {
        ...
        if (stage3Mode === 'select' && Array.isArray(stage3Reviewers) && stage3Reviewers.length > 0) {
          const wanted = stage3Reviewers.map((s) => s.toUpperCase());
          reviewFiles = reviewFiles.filter((rf) => wanted.includes(rf.initials.toUpperCase()));
          sendLog(`[info] Stage 3: select mode — ${reviewFiles.length} of ${wanted.length} requested reviewer file(s) found\n`);
        }

        if (reviewFiles.length > 0) {
          // Stage only the chosen files into a temp dir so the container
          // sees exactly those TSVs and nothing else.
          const stagedDir = path.join(outputDir, 'review_files_staged');
          await fsp.mkdir(stagedDir, { recursive: true });
          // Clear stale staged files from any previous run
          for (const f of await fsp.readdir(stagedDir).catch(() => [])) {
            await fsp.unlink(path.join(stagedDir, f)).catch(() => {});
          }
          for (const rf of reviewFiles) {
            await fsp.copyFile(rf.filePath, path.join(stagedDir, rf.filename));
          }

          const reviewDirInContainer = '/app/review_input';
          baseArgs.push('-v', `${stagedDir}:${reviewDirInContainer}:ro`);
          imageArgs.push('--review_dir', reviewDirInContainer);

          sendLog(`[info] Stage 3: ${reviewFiles.length} reviewer file(s) mounted for consensus\n`);
          for (const rf of reviewFiles)
            sendLog(`[info]   ${rf.initials} @ ${rf.timestamp}: ${rf.filename}\n`);
        } else {
          sendLog('[info] Stage 3: no matching reviewer files — falling back to automatic cleanup\n');
        }
      }
    }
