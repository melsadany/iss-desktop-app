...
1. **Record** the ISS task by playing the bundled task video fullscreen and capturing participant audio.
2. **Run** the analysis pipeline locally via the `melsadany/iowa_speech_sample:latest` Docker image.
3. **View** per-participant outputs — tabular files render in-app; other file types open from the data folder.

The app reuses the official Docker image, so feature extraction is identical to the CLI pipeline while providing a simple desktop interface for setup, recording, execution, and review.
...
| Path | Contents |
|---|---|
| `userData/participants/<id>/input/` | Recorded or imported audio files (`<id>_<timestamp>.mp3`, plus other supported audio formats). |
...
| `userData/config/task_template.yaml` | Editable copy of the pipeline config, copied from `resources/` on first launch. |
